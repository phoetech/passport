<template>
	<ul class="pagination" v-if="data.total > data.per_page">
		<li class="page-item pagination-prev-nav" v-if="data.prev_page_url">
			<a class="page-link" :href="getPageUrl(data.current_page-1)" aria-label="Previous" @click="selectPage(--data.current_page)">
				<slot name="prev-nav">
		...
		<li class="page-item pagination-page-nav" v-for="n in getPages()" :class="{'active': n == data.current_page }">
			<a class="page-link" :href="getPageUrl(n)" @click="selectPage(n)">{{ n }}</a>
		</li>
		<li class="page-item pagination-next-nav" v-if="data.next_page_url">
			<a class="page-link" :href="getPageUrl(data.current_page+1)" aria-label="Next" @click="selectPage(++data.current_page)">
				<slot name="next-nav">
...
module.exports = {
	...
	methods: {
		...
		getPageUrl: function(page) {
			let l = location.hash.replace(/(&?)page=([^&]*)/g, "");
			if (l == "" || l == "#") {
				return "#page=" + page;
			}
			return l + "&page=" + page;
